...
struct Vec256f3 {
    ...
    friend Vec256f3 operator*(const Vec256f3& v, float s) {
        __m256 ss = _mm256_set1_ps(s);
        return {_mm256_mul_ps(v.x, ss), _mm256_mul_ps(v.y, ss), _mm256_mul_ps(v.z, ss)};
    }
...
class Vec3f {
   public:
    ...
    Vec3f& operator/=(float t) { return *this *= 1 / t; }
...
inline Vec3f cross(const Vec3f& u, const Vec3f& v) {
    return Vec3f(u.e[1] * v.e[2] - u.e[2] * v.e[1], u.e[2] * v.e[0] - u.e[0] * v.e[2],
